...
from flat_utils import flat_to_dict
...
class Spider2(scrapy.Spider):
    ...
    def parse_item(self,
        ...
        self.logger.info('Got successful response from {}'.format(response.url))
        soup = BeautifulSoup(response.body, 'html.parser')
        print(flat_to_dict(soup))
        with open('flats.csv', 'a') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=flat_to_dict(soup).keys())
            writer.writerow(flat_to_dict(soup))
